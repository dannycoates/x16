...
const window = {
  BroadcastChannel: sinon.stub().returns(broadcastChannel)
}
...
const getInterface = {
  getInterface: sinon.stub().returns(docShell)
}
...
const Ci = {
  nsIInterfaceRequestor: 'sure',
  nsIDocShell: 'whatever'
}
const Services = {
  io: {
    newURI: sinon.stub().returns('baseURI://ok')
  },
  scriptSecurityManager: {
    createCodebasePrincipal: sinon.stub()
  },
  appShell: {
    createWindowlessBrowser: sinon.stub().returns(addonChromeWebNav)
  }
}
...
const WebExtensionChannel = proxyquire('../lib/metrics/webextension-channels', {
  'chrome': {
    Ci,
    '@noCallThru': true
  },
