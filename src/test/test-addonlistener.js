...
const AddonListener = proxyquire('../lib/actionCreators/AddonListener', {
  ...
  describe('event listeners', function () {
    ...
    describe('onOperationCancelled', function () {
      it('dispatches EXPERIMENT_ENABLED when addon has a pending enable', function () {
        ...
        assert.ok(s.dispatch.calledOnce)
        assert.equal(s.dispatch.firstCall.args[0].type, EXPERIMENT_ENABLED.type)
      })
      ...
      it('dispatches EXPERIMENT_DISABLED when addon is userDisabled and installed', function () {
        ...
        const l = new AddonListener(s)
        l.onOperationCancelled({ id: 'x', userDisabled: true, installDate: new Date() })
        assert.ok(s.dispatch.calledOnce)
        assert.equal(s.dispatch.firstCall.args[0].type, EXPERIMENT_DISABLED.type)
      })
    })
  })
})
