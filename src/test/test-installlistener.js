...
describe('InstallListener', function () {
  ...
  it('handles onInstallCancelled', function () {
    ...
    assert.ok(dispatch.calledOnce)
    assert.equal(dispatch.firstCall.args[0].type, actions.INSTALL_CANCELLED.type)
  })

  it('handles onDownloadStarted', function () {
    listener.onDownloadStarted(addonInstall)
    assert.ok(dispatch.calledOnce)
    assert.equal(dispatch.firstCall.args[0].type, actions.DOWNLOAD_STARTED.type)
  })

  it('handles onDownloadProgress', function () {
    listener.onDownloadProgress(addonInstall)
    assert.ok(dispatch.calledOnce)
    assert.equal(dispatch.firstCall.args[0].type, actions.DOWNLOAD_PROGRESS.type)
  })
  ...
  it('handles onDownloadCancelled', function () {
    ...
    assert.ok(dispatch.calledOnce)
    assert.equal(dispatch.firstCall.args[0].type, actions.DOWNLOAD_CANCELLED.type)
  })
