...
const AddonManager = {
  getAllAddons: sinon.stub().callsArgWith(0, addons)
}
...
const req = {
  on: sinon.stub().callsArgWithAsync(1, res),
  get: sinon.spy()
}
...
const Loader = proxyquire('../lib/actionCreators/Loader', {
...
describe('Loader', function () {
  ...
  describe('loadExperiments', function () {
    it('dispatches EXPERIMENTS_LOADED on success', function (done) {
      ...
      const l = new Loader(s)
      l.loadExperiments('test', 'foo')
       .then(
         () => {
           assert.ok(s.dispatch.calledTwice)
           assert.equal(s.dispatch.firstCall.args[0].type, MAYBE_NOTIFY.type)
           assert.equal(s.dispatch.secondCall.args[0].type, EXPERIMENTS_LOADED.type)
           done()
         },
         () => assert.fail()
       )
      ...
      l.loadExperiments('test', 'foo')
       .then(
         () => {
           ...
           assert.equal(store.dispatch.firstCall.args[0].type, EXPERIMENTS_LOAD_ERROR.type)
           done()
         },
         () => assert.fail()
       )
      ...
      l.loadExperiments('test', 'foo')
       .then(
         () => {
           ...
           assert.equal(s.dispatch.thirdCall.args[0].type, EXPERIMENTS_LOADED.type)
           done()
         },
         () => assert.fail()
       )
    ...
    it('does not call WebExtensionChannel.add on inactive experiments', function (done) {
      AddonManager.getAllAddons.callsArgWith(0, [{id: 'x', isActive: false, installDate: new Date()}])
      const s = {
        dispatch: sinon.spy(),
        getState: sinon.stub().returns({
          experiments: {},
